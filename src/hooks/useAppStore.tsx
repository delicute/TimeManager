    ...
    if (s.isActive && s.startTime) {
      ...
      try {
        ...
        if (activeType === 'Study' || activeType === 'Hobby') {
          ...
          if (earned > 0) window.electronAPI.notificationShow({ type: activeType, notifType: 'info', title: bl === 'zh' ? `赚取 ${earned} 余额` : `Earned ${earned}`, body: '', color: '#a09d96', duration: bc.notificationDuration ?? 5 });
